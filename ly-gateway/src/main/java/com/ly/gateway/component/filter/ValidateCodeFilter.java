...
import com.ly.common.util.AuthUtils;
import com.ly.common.util.R;
import com.ly.common.util.exception.ValidateCodeException;
...
public class ValidateCodeFilter extends ZuulFilter {
    ...
    @Value("${security.validate.code}")
    boolean checkCode;

    @Value("${security.validate.mobile-code}")
    boolean mobileCode;

    @Value("${security.validate.password-code}")
    boolean passwordCode;
    ...
    public Object run() {
        ...
        MobileLoginBody loginBody = new MobileLoginBody(request);

        String grantType = request.getParameter("grant_type");

        if(!passwordCode&&"password".equals(grantType)){
            return null;
        }
        if("mobile".equalsIgnoreCase(grantType)){
            return null;
        }

        if(loginBody.isMobile()){
            if(!mobileCode){
                //是否是跳过验证码
                return null;
            }else if(loginBody.isMobileWeChatLogin()){
                //判断是否是微信登录
                return null;
            }
        }
